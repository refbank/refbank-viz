...
harmonized_datasets <- list(games_df, rounds_df, player_inputs_df, survey_df, other_df) %>%
  map(~ .x %>% mutate_if(is.character, fill_na_with_mode) %>% mutate_if(is.numeric, fill_na_with_mean))

# Saving harmonized datasets
write.csv(chat_df,"harmonized_data.csv" )
...
write_csv(harmonized_datasets[[4]], "harmonized_survey.csv")
write_csv(harmonized_datasets[[5]], "harmonized_other_data.csv")
